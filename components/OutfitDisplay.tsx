...
export const OutfitDisplay: React.FC<OutfitDisplayProps> = ({ 
  ...
  return (
    <div className="animate-fade-in-up relative">
       {/* Refresh Button Positioned Top Right within the content area */}
       <div className="absolute -top-14 right-0 flex justify-end">
          <button
            onClick={onRefresh}
            className="flex items-center gap-2 px-4 py-2 bg-slate-800/80 hover:bg-indigo-600/80 border border-slate-700 hover:border-indigo-500 rounded-xl transition-all text-slate-300 hover:text-white backdrop-blur-md shadow-lg"
            title="Generate new variation"
          >
            <RefreshIcon />
            <span className="hidden sm:inline text-sm font-medium">Refresh Fit</span>
          </button>
       </div>
